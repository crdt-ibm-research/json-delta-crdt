...
describe("test frontend - utility functionality", () => {
  describe("check nesting map", () => {
    ...
    it("check empty nesting", () => {
      ...
      expect(ORMap.value(ormap)).to.deep.equal({ content: {} });
    });
  ...
  describe("check array proxy", () => {
    ...
    it("check init empty array", () => {
      ...
      callback(rootProxy);
      expect(ORMap.value(context.doc)).to.deep.equal({ a: [new Set([1])] });
    }),
      it("check add a value", () => {
        ...
        callback(rootProxy);
        expect(ORMap.value(context.doc)).to.deep.equal({ a: [new Set([2])] });
      });
  });
