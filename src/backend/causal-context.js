...
class CausalContext {
  ...
  next() {
    assert(
      this._id !== undefined,
      "id is undefined, deltas cc cannot be used to create dots"
    );
  ...
  join(other) {
    assert(
      other instanceof CausalContext,
      "join should receive a causal context"
    );
    ...
    for (let key of allKeys) {
      this._cc.set(
        key,
        Math.max(this._cc.get(key) || 0, other._cc.get(key) || 0)
      );
    }
