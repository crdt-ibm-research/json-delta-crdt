...
class DotFun {
  ...
  static join([m1, cc1], [m2, cc2]) {
    ...
    assert(
      m1 instanceof DotFun,
      "left hand dotstore is not an instance of DotFun"
    );
    assert(
      m2 instanceof DotFun,
      "right hand dotstore is not an instance of DotFun"
    );
    assert(cc1 instanceof CausalContext, "left hand has invalid CausalContext");
    assert(
      cc2 instanceof CausalContext,
      "right hand has invalid CausalContext"
    );
    ...
    for (const dot of allDots) {
      if (m1.has(dot) && m2.has(dot)) {
        ...
        assert(
          m1.get(dot) === m2.get(dot),
          "Error - both dotstores contain the same dot with different value"
        );
...
class DotFunMap {
  ...
  static join([m1, cc1], [m2, cc2]) {
    ...
    assert(
      m1 instanceof DotFunMap,
      "left hand dotstore is not an instance of DotFunMap"
    );
    assert(
      m2 instanceof DotFunMap,
      "right hand dotstore is not an instance of DotFunMap"
    );
    assert(cc1 instanceof CausalContext, "left hand has invalid CausalContext");
    assert(
      cc2 instanceof CausalContext,
